...
seed_lexicon = {
    "음식": [
        "맛", "음식", "메뉴", "맛집", "반찬", "식사", "요리", "종류", "구성", "양", "퀄리티", "식재료", "디저트",
    ],
    "서비스": [
        "친절", "사장", "직원", "추가", "서비스", "응대", "배려", "알바", "이모",
    ],
    "분위기": [
        "분위기", "느낌", "인테리어", "편안", "매장", "힐링", "레트로", "추억", "여유", "위생", "정신",
        "음악", "노래", "뷰", "청결"
    ],
    "가격": [
        "가성비", "가격", "혜자", "물가", "값", "비용", "금액"
    ],
    # 식당 시설, 영업 시간, 대기 시간 등
    "편의성": [
        "주차", "주차장", "화장실", "시간", "예약", "브레이크", "만석", "웨이팅", "대기", 
        "회전율", "콘센트", "혼밥", "좌석", "자리", "테이블", "메뉴판"
    ]
}
...
# 감정 사전의 기준 단어입니다.
pos_seeds = [
    "맛있", "좋", "많", "맛나", "편하", "넓", "괜찮", "예쁘", "크", "배부르", "이쁘", "잘", "추천",
    "저렴하", "착하", "알차", "재밌", "든든하", "멋지", "빠르", "푸짐하", "야무지", "빨리", "금방",
    "재미있", "실하", "기쁘", "멋있", "알맞", "좋아하", "즐기", "어울리", "생각나", "깔끔",
    "땡기", "어우러지", "끝내주", "반하", "짱", "제대로", "가득", "듬뿍", "바삭", "야들야들",
    "최고", "친절하", "신선하", "넉넉",
]
neg_seeds = [
    "늦", "적", "작", "비싸", "힘들", "느끼하", "나쁘", "질기", "과하", "비리", "맛없", "싫", "땡볕",
    "지옥", "소음", "딱딱", "퍽퍽하", "정신없", "속상하", "질리", "물리", "싫어하", "더럽", "덥", "춥",
    "상하", "별로", "그닥", "전혀", "아예", "최악", "아쉽", "당황", "답답", "오르", "제한", "기다리",
    "엉망", "실망", "불만", "불친절", "오래", "느린", "안됨", "부족", "솔드아웃", "매진",
]
...
for word, score in sentiment_lexicon.items():
    if -1.0 <= score < -0.5: neg_count += 1;
    elif -0.5 <= score <= 0.5: neu_count += 1;
    elif 0.5 <= score <= 1.0: pos_count += 1;

print("\n=== 감정 사전 분포 ===")
print(f"부정 [-1.0 ~ -0.5] : {neg_count} words ({neg_count/total_words*100:.1f}%)")
print(f"중립 [-0.5 ~  0.5] : {neu_count} words ({neu_count/total_words*100:.1f}%)")
print(f"긍정 [ 0.5 ~  1.0] : {pos_count} words ({pos_count/total_words*100:.1f}%)")
